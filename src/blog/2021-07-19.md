---
title: 初心者でもコンパイルがしたい方へ
date: 2021-07-19 13:00:00
excerpt:
type: post
blog: true
tags:
...
普段JSやPUG,SCSSをコンパイルするのに一括で管理するためにgulpやwebpackさらにはviteなんかも使っているかと思います。

しかし、チームのみんなが設定ファイルを管理し修正すると言うのは少し難しく感じます。

そこで今日は超簡単にそれらをコンパイルするPreprosを教えたいと思います！！

## Prepros

[https://prepros.io](https://prepros.io)

- SCSS, Pug, Stylus, Less, Slim, TypeScript主要なものはコンパイル可能。
- ローカルサーバーにて表示可能
- ファイル監視によるライブリロード
- 同じIPに接続してるモバイル端末でも表示可能
- GUIを使用して操作するから楽々


## 使い方

1, 上記のサイトからアプリをダウンロード

2, prepros.configを対象のルートディレクトリに配置

3, OPEN Previewをクリック
![hoge](https://pedantic-goldberg-e70663.netlify.app/image/server.png)  
...
## まとめ
実際の現場ではあまり支流なものではないので、webpackなどを学ぶ必要はあるかと思いますが受託の案件などささっと作りたい場合は実務でも十分に使えるものだと感じました。